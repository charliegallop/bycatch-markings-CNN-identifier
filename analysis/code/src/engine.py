...
class engine():
    ...
    def __init__(self, BACKBONE, TRAIN_FOR, EPOCHS):
        ...
        self.lr_scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, 3, gamma = 0.1)
    ...
    def validate(self, valid_data_loader, model):
        ...
        return self.val_loss_epoch.value
    ...
    def run(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            self.val_loss_all.append(self.val_loss_epoch.value)
            ...
            #     train_ax.plot(summed_losses,epoch+1, color = 'blue')
            ...
            #     train_ax.plot(summed_losses, color = 'blue')
            #     train_ax.set_xlabel('Epochs')
            #     train_ax.set_ylabel('train loss')
            #     valid_ax.plot(val_loss, color='red')
            #     valid_ax.set_xlabel('Epochs')
        ...
        model_path = os.path.join(ROOT, 'outputs', self.backbone, self.train_for, f'model{self.num_epochs}.pth')
        ...
        # print([*range(1, self.num_epochs+1,1)], self.train_loss_all, self.val_loss_all)
        loss_df = pd.DataFrame()
        loss_df['epochs'] = [*range(1, self.num_epochs+1,1)]
        loss_df['train_loss'] = self.train_loss_all
        loss_df['val_loss'] = self.val_loss_all

        loss_df.to_csv(f"{self.output_dir}/losses_df.csv")
        train_ax.plot(loss_df['train_loss'], color = 'blue')
        train_ax.set_xlabel('Epochs')
        train_ax.set_ylabel('train loss')

        valid_ax.plot(loss_df['val_loss'], color='red')
        valid_ax.set_xlabel('Epochs')
        valid_ax.set_ylabel('validation loss')
