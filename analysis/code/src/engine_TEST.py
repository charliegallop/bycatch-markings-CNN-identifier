from config import DEVICE, NUM_CLASSES_DOLPHIN, NUM_CLASSES_MARKINGS, NUM_EPOCHS, OUT_DIR, BACKBONE 
from config import THRESHOLD, CLASSES_DOLPHIN,CLASSES_MARKINGS, COLOURS, SELECTION
...
class engine():
    ...
    def calc_metrics(self):
        ...
        for i in range(len(test_images)):
            ...
            with torch.no_grad():
                outputs = self.model(image) # outputs will consit of 3 tensors [boxes:, labels:, scores:]
        ...
        if __name__ == '__main__':
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                if (epoch+1) % SAVE_MODEL_EPOCH == 0: # save the model after every n epoch
                    torch.save(self.model.state_dict(), f"{OUT_DIR}/model{epoch+1}.pth")
